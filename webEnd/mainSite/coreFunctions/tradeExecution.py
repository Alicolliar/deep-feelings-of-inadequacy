from mainSite.coreFunctions import cashTransactions, gettingData as gettingData
...
def tradeSubmit(cur, tradeType, ticker, volume, price, userSubmitting, manual=False):
    ...
    if tradeType == "Buy":
        enough = gettingData.checkCashValue(cur, userSubmitting, totalValue)
    elif tradeType == "Sell":
        enough = gettingData.checkPortValue(cur, userSubmitting, ticker, volume)
    if enough:
        return enough
    if not manual:
